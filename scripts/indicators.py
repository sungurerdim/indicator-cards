...
def kama(x, n=10, fast=2, slow=30):
    ...
    if len(x) <= n:
        return out
    change = np.abs(x - np.roll(x, n)); change[:n] = np.nan
...
def t3(x, n=20, a=0.7):
    ...
    c1 = -a**3; c2 = 3*a**2 + 3*a**3
    c3 = -6*a**2 - 3*a - 3*a**3; c4 = 1 + 3*a + a**3 + 3*a**2
...
def _di(df, n=14):
    ...
    pdi = 100 * pd.Series(pdm).ewm(alpha=1/n, adjust=False).mean().values / np.maximum(atr_s, 1e-9)
    ndi = 100 * pd.Series(ndm).ewm(alpha=1/n, adjust=False).mean().values / np.maximum(atr_s, 1e-9)
    return pdi, ndi
...
def _ichimoku_tk_state(d, t=9, k=26):
    ...
    st = np.sign(np.nan_to_num(v)); st[:k] = 0
    return st, v
...
def _ichimoku_kumo_state(d, t=9, k=26, b=52):
    ...
    ten = (h.rolling(t).max() + l.rolling(t).min()).values / 2
    kij = (h.rolling(k).max() + l.rolling(k).min()).values / 2
    spa = np.roll((ten + kij) / 2, k); spa[:k] = np.nan
    spb = np.roll((h.rolling(b).max() + l.rolling(b).min()).values / 2, k)
    spb[:k] = np.nan
    c = d['close'].values
    hi = np.maximum(spa, spb); lo = np.minimum(spa, spb)
...
def _adx_di_state(d, n=14):
    pdi, ndi = _di(d, n)
    v = (pdi - ndi) / 100
    st = np.sign(v); st[:2 * n] = 0
    return st, v


def _linreg_state(d, n=20):
    t = np.arange(n, dtype=float); w = t - t.mean(); w /= (w ** 2).sum()
...
def _stoch_state(d, n=14, sm=3):
    ...
    v = kk - 50
    st = np.sign(np.nan_to_num(v)); st[:n + sm] = 0
    return st, v
...
def _stochrsi_state(d, n=14, sm=3):
    ...
    v = sr - 0.5
    st = np.sign(np.nan_to_num(v)); st[:2 * n] = 0
    return st, v
...
def _tsi_state(d, r=25, s=13):
    ...
    st = np.sign(np.nan_to_num(v)); st[:r + s] = 0
    return st, v
...
def _trix_state(d, n=15):
    ...
    v = np.zeros(len(c)); v[1:] = e3[1:] / e3[:-1] - 1
    st = np.sign(v); st[:3 * n] = 0
    return st, v
...
def _kst_state(d):
    c = d['close'].values
    def roc(n):
        r = np.zeros(len(c)); r[n:] = c[n:] / c[:-n] - 1
        return r
    k = (sma(roc(10), 10) + 2 * sma(roc(15), 10)
         + 3 * sma(roc(20), 10) + 4 * sma(roc(30), 15))
    st = np.sign(np.nan_to_num(k)); st[:45] = 0
    return st, k
...
def _ultosc_state(d, n1=7, n2=14, n3=28):
    ...
    def avg(n):
        return (pd.Series(bp).rolling(n).sum()
                / pd.Series(tr).rolling(n).sum().replace(0, np.nan)).values
    uo = 100 * (4 * avg(n1) + 2 * avg(n2) + avg(n3)) / 7
    v = uo - 50
    st = np.sign(np.nan_to_num(v)); st[:n3] = 0
    return st, v
...
def _ao_state(d):
    ...
    st = np.sign(np.nan_to_num(v)); st[:34] = 0
...
def _fisher_state(d, n=9):
    ...
    val = np.zeros(len(mp)); fish = np.zeros(len(mp))
    for i in range(1, len(mp)):
        val[i] = 0.66 * raw[i] + 0.34 * val[i-1]
        x = np.clip(val[i], -0.999, 0.999)
        fish[i] = 0.5 * np.log((1 + x) / (1 - x)) + 0.5 * fish[i-1]
    st = np.sign(fish); st[:n] = 0
    return st, fish


def _cmf_state(d, n=20):
    h, l, c, v = (d['high'].values, d['low'].values,
                  d['close'].values, d['volume'].values)
    mfm = ((c - l) - (h - c)) / np.where((h - l) > 0, h - l, np.nan)
    mfv = np.nan_to_num(mfm) * v
    cmf = (pd.Series(mfv).rolling(n).sum()
           / pd.Series(v).rolling(n).sum().replace(0, np.nan)).values
    st = np.sign(np.nan_to_num(cmf)); st[:n] = 0
    return st, cmf
...
def _adl_state(d, n=20):
    ...
    st = np.sign(np.nan_to_num(sl)); st[:n] = 0
    return st, sl
...
def _eom_state(d, n=14):
    ...
    eom = pd.Series(dm / np.maximum(box, 1e-12)).rolling(n).mean().values
    st = np.sign(np.nan_to_num(eom)); st[:n] = 0
    return st, eom


def _vwap_pos_state(d):
    """Weekly-anchored VWAP relative position (resets each ISO week)."""
...
def _ext_states():
    S = []
    for n in (50, 200):
        S.append((f"sma_{n}_pos", lambda d, n=n: _ma_state(d, sma(d['close'].values, n))))
    for n in (20, 50):
        S.append((f"wma_{n}_pos", lambda d, n=n: _ma_state(d, wma(d['close'].values, n))))
    S.append(("kama_10_pos", lambda d: _ma_state(d, kama(d['close'].values))))
    S.append(("t3_20_pos", lambda d: _ma_state(d, t3(d['close'].values))))
